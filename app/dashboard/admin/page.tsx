...
export default function AdminDashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      setRefreshing(true);
      const [ordersResponse, statsResponse, dashboardResponse] =
        await Promise.all([
  ...
  const stationPerformance = mockStations.map((station) => {
    const stationOrders = orders.filter(
      (order) => order.machineId === station.id,
    );
    const stationRevenue = stationOrders.reduce(
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-600">
              {ongoingRentals} ongoing • {Number(rentalsChange) >= 0 ? "+" : ""}
              {rentalsChange}% from yesterday
            </p>
          </CardContent>
        </Card>
            ...
            {systemHealthData.map((metric) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <div
                    className={`w-3 h-3 rounded-full ${getHealthBg(metric.status)}`}
                  ></div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <span
                    className={`text-sm font-medium ${getHealthColor(metric.status)}`}
                  >
                    {metric.value}%
                  </span>
                </div>
      ...
      <Card className="hidden">
        <CardHeader>
          ...
          <CardDescription>
            Revenue and rental activity across all stations today
          </CardDescription>
        </CardHeader>
        <CardContent>
